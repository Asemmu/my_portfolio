      ...
      <div className='w-full flex justify-between items-center max-w-7xl mx-auto'>
        <Link to="/" className='flex items-center gap-2' onClick={() => {
          ...
          window.scrollTo(0, 0);
            ...
            <p className='text-white text-[18px] font-bold cursor-pointer flex'>Asem &nbsp;<span className='sm:block hidden'>Musleh</span></p>
        </Link>
        <ul className='list-none hidden sm:flex flex-row gap-10'>
        <li key={'cv'} className={` hover:text-white text-[18px] font-medium cursor-pointer`}>
              <motion.a transition={{duration: 3,repeat: Infinity, repeatType: 'loop',}}  animate={{ color: ['rgb(145,94,240)', '#fff'], startOffset: 0 }} href={`CVofAsemMosleh.pdf`} download>Download My CV</motion.a>
              </li>
          {navLinks.map((link) => 
